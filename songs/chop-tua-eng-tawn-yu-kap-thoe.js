 ...
 * 歌词文本：使用者自己提供的那份泰文歌词。
 *
 * ⚠️ 时间轴（下面的 TIMES）是脚本自动跑出来的草稿，不是逐句手工核对过的：
 *   用 scripts/auto-align-lyrics.py（Whisper 文字比对 + aeneas 强制对齐互相
 *   印证，细节见 scripts/auto-align-lyrics.md）对着官方 MV 跑出来的。
 *   副歌反复唱了三遍、结尾又复用一遍副歌歌词，这首歌里能两边互相印证、
 *   真正判定为「高精度」的只有第一遍副歌（c1）；其余段落大多是靠 aeneas
 *   兜底或插值出来的，比精确逐句校准要粗一截，还没有人工核对过。
 *   要修正：打开网页 → 右上角「⏱ 校准」，跟着歌每句按一下空格，一遍就好，
 *   结果自动存在浏览器里（会盖掉这里的值）。想固化进文件：校准面板里点
 *   「导出给别人用」，把导出来的那一块贴回 TIMES，再把下面的 synced 改成
 *   true，提示条就不会再出现了。
 *
...
(() => {
  ...
  /* ── 时间轴（秒）── 每句开唱的时间，按段落分开写，顺序 = 段落里句子的顺序。
     ...
     ⚠️ 这份是脚本自动跑出来的草稿，见文件顶部说明；只有 c1 是高精度结果，
     其余段落建议用网页上的「⏱ 校准」重新核一遍。 */
  const TIMES = {
    c1: [12.53, 17.43, 21.11, 27.61, 32.07, 35.49],   // 副歌
    v1: [42.69, 46.09, 49.83, 53.59, 56.89, 60.79, 64.29, 68.60],   // 主歌 A
    p1: [72.92, 77.23, 81.54],   // 预副歌
    c2: [85.86, 90.17, 93.83, 101.56, 109.28, 117.01, 124.73],   // 副歌
    v2: [132.46, 140.18, 147.91, 155.63],   // 主歌 B
    p2: [163.36, 171.08, 178.81],   // 预副歌
    c3: [186.53, 194.26, 201.98, 209.71, 217.43, 225.16, 232.88],   // 副歌
    o1: [240.61, 244.89, 248.99, 253.09],   // 结尾
  };
