 ...
 *
 * ⚠️ 这首歌只做练习模式：timeline: false
 *   —— 没有逐句时间轴，页面不会自动跟着原曲高亮，自己手动往下滑。
 *   原曲播放器还在（可以边听边看），只是不做逐句同步、不做 KTV、不做校准。
 *   以后想加时间轴：把 timeline 改成 true，给每句补 start（秒），
 *   然后用右上角「⏱ 校准」标一遍就行。
 *
...
(() => {
  ...
  // 补 id 和 lang：id 用「段落前缀-第几句」，全曲唯一
  const withIds = (prefix, lines) =>
    lines.map((l, i) => Object.assign({ id: `${prefix}-${i + 1}`, lang: 'th' }, l));
  ...
  window.SONGS['hua-jai-sang-hai-rak-thoe'] = {
    ...
    // ⚠️ 只做练习模式：没有逐句时间轴，页面不自动跟随，自己手动滑
    timeline: false,
